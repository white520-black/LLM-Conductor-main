...
class HubOperator:
    # 关键修改1：增加 langchain_memory 参数，接收跨任务记忆模块
    def __init__(self, tool_importer, memory_obj, langchain_memory=None):
        ...
        # 关键修改2：存储 LangChain 记忆模块（用于跨任务信息复用）
    ...
    # 关键修改3：增加 current_max_tokens 参数，传递动态计算的最大tokens
    def run(self, query, plan, current_max_tokens=None):
        self.query = query
        ...
        if num_step_list == 0:
            if self.vanilla_spoke is None:
                # 关键修改4：创建 VanillaSpoke 时传递记忆模块和动态max_tokens
                self.vanilla_spoke = VanillaSpoke(
                    langchain_memory=self.langchain_memory,
                    current_max_tokens=current_max_tokens
                )
            # 关键修改5：执行时传递动态max_tokens
            results = self.vanilla_spoke.llm_execute(query, current_max_tokens=current_max_tokens)

            # Apps are executed in a cascaded manner
        elif num_step_list == 1:
            startup_app = self.plan[0][0]['name']
            # 关键修改6：传递 current_max_tokens 给初始Spoke
            consent, results = self.run_initial_spoke(query, startup_app, current_max_tokens=current_max_tokens)

        # Apps can be executed concurrently, use a dedicated spoke for routing the query
        else:
            startup_app = ""
            # 关键修改7：传递 current_max_tokens 给初始Spoke
            consent, results = self.run_initial_spoke(query, startup_app, current_max_tokens=current_max_tokens)
    ...
    # 关键修改8：增加 current_max_tokens 参数，传递给执行逻辑
    def run_initial_spoke(self, query, startup_app, current_max_tokens=None):
        if startup_app:
        ...
        if not consent:
            ...
            if all_empty:
                entities = ""
            # 关键修改9：传递 current_max_tokens 给 execute_app_spoke
            results = self.execute_app_spoke(
                query, entities, startup_app, flag=False,
                current_max_tokens=current_max_tokens
            )

        return consent, results

    # 关键修改10：增加 current_max_tokens 参数，传递给Spoke实例
    def execute_app_spoke(self, query, entities, requested_app, flag=False, current_max_tokens=None):
        ...
        if requested_app in self.spoke_tool.keys():
            ...
            # 关键修改11：传递 langchain_memory 和 current_max_tokens 给 Spoke.run_process
            p = multiprocessing.Process(
                target=spoke.run_process,
                args=(child_sock, query, spoke_session_id, entities, self.langchain_memory, current_max_tokens)
            )
            p.start()
            ...
            # 关键修改12：创建 Spoke 时传递记忆模块和动态max_tokens
            spoke = Spoke(
                tool=None, functionalities=[], flag=flag,
                langchain_memory=self.langchain_memory,
                current_max_tokens=current_max_tokens
            )
            self.spoke_counter += 1

            # 关键修改13：传递 langchain_memory 和 current_max_tokens 给 Spoke.run_process
            p = multiprocessing.Process(
                target=spoke.run_process,
                args=(child_sock, query, spoke_session_id, entities, self.langchain_memory, current_max_tokens)
            )
            ...
            session_id = uuid.uuid4()
            spoke_session_id = self.user_id + ":" + str(self.spoke_counter) + ":" + str(session_id)
            # 关键修改14：创建 Spoke 时传递记忆模块和动态max_tokens
            self.spoke_tool[requested_app] = {
                'id': self.spoke_counter,
                'spoke': Spoke(
                    tool=tool, functionalities=tool_functionalities, spec=tool_spec, flag=flag,
                    langchain_memory=self.langchain_memory,
                    current_max_tokens=current_max_tokens
                ),
                'tool': tool
            ...
            # 关键修改15：传递 langchain_memory 和 current_max_tokens 给 Spoke.run_process
            p = multiprocessing.Process(
                target=spoke.run_process,
                args=(child_sock, query, spoke_session_id, entities, self.langchain_memory, current_max_tokens)
            )
        ...
        while True:
            ...
            if data['message_type'] == 'app_request':
                ...
                if functionality_request not in self.function_tools.keys():
                    ...
                    # 关键修改16：传递 current_max_tokens 给嵌套的 execute_app_spoke 调用
                    app_response = self.execute_app_spoke(
                        str(data), entities, tool, False,
                        current_max_tokens=data.get('current_max_tokens')  # 从请求中获取max_tokens
                    )
